...
/**
 * @author alekstheod Represent the loading frame in ProMasi system. Needed in
 * order to present the loading process in case of network interaction -
 * downloads, game start loading and so on.
 */
public class LoadingJPanel extends JPanel implements IClientGameListener {

    /**
     * Instance of {@link = IGame} interface implementation, which represent the
     * game on ProMaSi system.
     */
    private IGame _game;

    /**
     * Instance of {@link = IMainFrame} interface implementation, which
     * represent the main frame in ProMasi system.
     */
    ...
    public LoadingJPanel(IMainFrame mainFrame, IGame game, String username) throws GuiException {
        if (mainFrame == null || username == null || username.isEmpty()) {
