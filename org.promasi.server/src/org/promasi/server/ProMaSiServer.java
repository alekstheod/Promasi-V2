...
/**
 *
 * @author m1cRo Represent the server in ProMaSi system. Server is responsible
 * to manage all the running online games.
 */
public class ProMaSiServer implements IPromasiClientListener, ITcpServerListener {

    /**
     * The tcp server needed in order to handle the tcp connections.
     */
    private TcpServer _server;

    /**
     * List of connected clients. In order to connect client should call the
     ...
     * a new game, in order to create a new game client has to call the
    ...
    /**
     * Instance of {@link ILogger} interface implementation, needed for logging.
     */
    ...
    /**
     * If the ProMaSi server is stopped all the available clients will be
     * disconnected.
     */
    @Override
    ...
    /**
     * Login method, user must call this method in order to join to online games
     * room. If the an user with the given userId already registered in the
     * system this method will fail and return false, otherwise user will be
     * registered. Call this method before any method you will use with the
     * given userId.
     *
     * @param userId Game player identification string, actually the username of
     * the player.
     * @param client Instance of {@link = ProMaSiClient} which represent the
     * game player.
     * @return true in case if login succeed, false otherwise.
     */
    ...
    /**
     * Client should call this method in order to start the prepared by him
     * game.
     *
    ...
    /**
     * User can call this method in order to check if the ProMasiServer is
     ...
     *
     * @return instance of {@link Map} interface implementation which contains
     * the pairs of gameId and game description strings.
     */
    ...
    /**
     * User should call this method in order to join to the ProMaSi game.
     *
     ...
     * @throws NetworkException In case of invalid arguments ( Game with the
     * given game Id is not available, null arguments and so on).
     */
    ...
    private boolean sendUpdateGamesListRequest(ProMaSiClient client) {
        ...
        try {
            _lockObject.lock();
            if (client != null) {
                Map<String, String> gamesList = new TreeMap<>();
                for (Map.Entry<String, MultiPlayerGame> entry : _availableGames.entrySet()) {
                    gamesList.put(entry.getKey(), entry.getValue().getGameDescription());
                }

                result = client.send(new UpdateGameListRequest(gamesList));
            ...
            if (gameId != null) {
                if (_availableGames.containsKey(gameId)) {
                    ...
                    for (String playerId : gamePlayers) {
                        if (!playerId.equals(game.getGameOwnerId()) && _clients.containsKey(playerId)) {
                            _clients.get(playerId).send(new GameCanceledRequest());
                        }
                    }

                    if (_clients.containsKey(game.getGameOwnerId())) {
                        _clients.get(game.getGameOwnerId()).send(new CancelGameResponse());
                    }
    ...
    /**
     * User will call this method in order to leave the joined game. User will
     * be removed from the game players list. An UpdateGamePlayersListRequest
     * message will be sent to all the players of the given name.
     *
            ...
            if (gameId != null && clientId != null) {
                if (_availableGames.containsKey(gameId)) {
                    ...
                    if (gamePlayers.contains(clientId) && _clients.containsKey(clientId)) {
                        _clients.get(clientId).send(new LeaveGameResponse());
                        gamePlayers.remove(clientId);
                        for (String player : gamePlayers) {
                            if (_clients.containsKey(player)) {
                                result = _clients.get(clientId).send(new UpdateGamePlayersListRequest(gamePlayers));
    ...
    /**
     * In case of an client was disconnected from the server we have to remove
     * him from the clients list.
     */
    @Override
    public void onDisconnect(ProMaSiClient client) {
        try {
            _lockObject.lock();
            Map<String, ProMaSiClient> clients = new TreeMap<>(_clients);
