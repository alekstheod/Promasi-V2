...
/**
 * @author m1cRo Represent the Multiplayer game in ProMaSi system. This game
 * contains a list of the running game models, one for each client.
 */
public class MultiPlayerGame implements IMultiPlayerGame, IClockListener, IGameModelListener, IEmployeeListener, ICompanyListener, IDepartmentListener {
     ...
     */
    private final Map<String, GameModel> _gameModels = new TreeMap<>();

    /**
     *
     */
    private final Map<String, GameModelModel> _finishedGames = new TreeMap<>();
    ...
    /**
     * Instance of {@link = ILogger} interface implementation. Needed for
     * logging.
     */
    ...
    public boolean startGame(final String clientId) {
        ...
        try {
            ...
            if (!_isRunning && _gameModels.containsKey(clientId)) {
                ...
                _isRunning = true;
                result = true;
                LinkedList<IServerGameListener> listeners = new LinkedList<>(_listeners);
    ...
    public List<String> getGamePlayers() {
        try {
            _lockObject.lock();
            List<String> players = new Vector<>();
            for (Map.Entry<String, GameModel> entry : _gameModels.entrySet()) {
